...
require('connect.php');
    ...
    && isset($_POST['pin']))
    {
        ...
        $query = "UPDATE `users` SET `user_name`='$username',`email`='$email',`mobile`=$mobile,`password`='$password',`address1`='$address1',`address2`='$address2',`city`='$city',`state`='$state',`pin`=$pin
        where user_id=$uid";
        echo $query;
